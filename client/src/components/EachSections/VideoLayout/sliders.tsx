        ...
        {(cues, forwardRef) =>
          cues.map((cue) => (
            <div
              ...
              ref={forwardRef}
            >
              <TimeSlider.Track className="relative ring-media-focus z-0 h-[5px] w-full rounded-sm bg-white/25 group-data-[focus]:ring-[3px]">
                <TimeSlider.TrackFill className="bg-white absolute h-full w-[var(--chapter-fill)] rounded-sm will-change-[width]" />
                <TimeSlider.Progress className="absolute z-10 h-full w-[var(--chapter-progress)] rounded-sm bg-white/25 will-change-[width]" />
              </TimeSlider.Track>
            </div>
          ))
        }
